...
```
docker run --rm -it -v ~/.config/gcloud:/root/.config/gcloud -v ~/.aws:/root/.aws -v ~/.azure:/root/.azure -v ~/.kube:/root/.kube -v ~/.config/gh:/root/.config/gh -v ~/.ssh:/root/.ssh -v ~/hangar_workspace:/scripts/workspace -v ~/.config/configstore:/root/.config/configstore hangar bash
```
