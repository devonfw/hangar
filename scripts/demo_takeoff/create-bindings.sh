...
then
	...
	then
	    gcloud projects add-iam-policy-binding $project_id --member=serviceAccount:$service_account_email --role=$i
	    if ! [ $? -eq 0 ]
	    then
		echo -e "${red}Error: Attaching role $i to $service_account_email for project $project_id.." >&2
		exit 2
	    fi
	fi
    done
...
then	
    ...
    then
        gcloud projects add-iam-policy-binding $project_id --member=serviceAccount:$service_account_email --role=projects/$project_id/roles/$custom_role_id
    fi
fi
