...
az pipelines variable-group variable list --organization "$1" --project "$2" --group-id "$groupid" | grep -q "^$4" || az pipelines variable-group variable update --organization "$1" --project "$2" --group-id $groupid --name "$4" --value "$5"
