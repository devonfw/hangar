...
kubectl create secret docker-registry "$2" --docker-server="$5" --docker-username="$3" --docker-password="$4" --namespace="$1" --kubeconfig "$7" 
...
# Apply the changes.
kubectl apply -f "$6" --namespace="$1" --kubeconfig "$7"
