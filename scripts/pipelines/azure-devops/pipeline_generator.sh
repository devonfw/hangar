#!/bin/bash
ARGS=$*
FLAGS=$(getopt -a --options c:n:d:a:b:l:i:u:p: --long "config-file:,pipeline-name:,local-directory:,artifact-path:,target-branch:,language:,build-pipeline-name:,sonar-url:,sonar-token:,image-name:,user:,password:,resource-group:,storage-account:,storage-container:,cluster-name:,s3-bucket:,s3-key-path:,deploy-files:,aks-provision-pipeline:,deploy-cluster:,k8s-namespace:,secrets-value:,package-pipeline-name:," -- "$@")
eval set -- "$FLAGS"
while true; do
    case "$1" in
        ...
        --aks-provision-pipeline)    aksProvisionPipelineName=$2; shift 2;; 
        --deploy-cluster)            deployCluster=$2; shift 2;; 
        --k8s-namespace)             k8sNamespace=$2; shift 2;; 
        --secrets-value)             secretsValue=$2; shift 2;;  
...
function help {
    ...
    echo "      --k8s-namespace			    [Required] Name of the kubernetes namespace."
    echo "      --secrets-value                        Name of the secrets, pull images from private registry."
    echo "      --package-pipeline-name     [Required] Name of the Package pipeline name."
    echo ""
