...
pipeline_generator.sh \
  ...
  [-t <target-directory>] \
  [-b <branch>] \
  [-m <machine type for the {pipeline_type}>]
```
...
-m, --machine-type                  Option for the machine type to have a powerful VM for your {pipeline_type} if the default one is not enough. Accepted values: 'E2_HIGHCPU_8', 'E2_HIGHCPU_32', 'N1_HIGHCPU_8' or 'N1_HIGHCPU_32'.
```
