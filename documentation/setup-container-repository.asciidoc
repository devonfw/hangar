...
* In this block, we will see how to setup/create container registry to pull/tag/push container images. 
...
* Here is the container image name format

`[registry-name]/[project-id]/[image]:[tag]`

* `<hostname>` : <aws-account-id>.dkr.ecr.<region>.amazonaws.com
** `<aws-account-id>` : AWS Account ID which is unique for each account.
** `<region>` : AWS Region which is a separate geographic area and the examples are us-east-1, eu-west-1, ap-south-1. 
* `<project-id>/<image>` : Repository name given on creation.
...
* https://docs.microsoft.com/en-us/azure/container-registry/container-registry-get-started-portal[ACR] is a private registry service from Azure Cloud^[4]^.
