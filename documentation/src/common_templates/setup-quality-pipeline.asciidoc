...
ifdef::test-pipeline[  --test-pipeline-name <test pipeline name> \]
  [-b <branch>] \
  [-w]
```
...
ifdef::test-pipeline[    --test-pipeline-name     [Required] Test pipeline name.]
...
ifdef::test-pipeline[--test-pipeline-name node-project-test ]
```
