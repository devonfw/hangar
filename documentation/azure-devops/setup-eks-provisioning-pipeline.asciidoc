...
After successfull execution of the script, EKS cluster is provisioned. And also a variable group with the name `eks-variables` is created. This contains the DNS name of the nginx ingress controller deployed on the EKS cluster. The DNS name may be used in the application ingress manifest files.

Rancher can be accessed at the DNS name of the nginx ingress controller. Refer the appendix to retrieve the DNS name from the nginx ingress controller. 
```
