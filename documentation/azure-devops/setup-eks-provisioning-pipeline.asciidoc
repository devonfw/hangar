...
-d, --local-directory    [Required] Local directory of your project (the path should always be using '/' and not '\').
    ...
    --cluster-operation             Operation to be performed on EKS cluster to create or destroy it. Default is create.
...
To destroy the resources provisioned, pass the clusterOperation parameter value as `destroy`.

=== Example

```
./pipeline_generator.sh -c ./templates/eks/eks-pipeline.cfg -n eks-provisioning -d C:/Users/$USERNAME/Desktop/quarkus-project --cluster-name hangar-eks-cluster --s3-bucket terraformStateBucket --s3-key-path eks/state -b develop -w --cluster-operation destroy
```
