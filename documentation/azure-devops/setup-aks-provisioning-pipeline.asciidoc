...
=== Appendix: Installing Rancher

In order to install Rancher to manage your Kubernetes cluster execute the `pipeline_generator.sh` script with the `--rancher` flag:
...
To get the domain name in order to access Rancher go into Azure DevOps > Pipelines > Library > aks-variables.

You will be able to access the Rancher dashborad through the URL: https://<domain name>.

When accessing the URL you will be asked to introduce the inital password which you can obtain bey executing the following command on your console:
...
To destory the provisioned resources, set `operation` pipeline variable value to `destroy` and run the pipeline.