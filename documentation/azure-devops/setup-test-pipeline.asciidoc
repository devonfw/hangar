...
The script located at `/scripts/pipelines/{provider}/pipeline_generator.sh` will automatically create new branch, create a test {pipeline_type} based on a YAML template appropriate for the project programming language or framework, create the Pull Request, and if it is possible, merge this new branch into the specified branch.
...
NOTE:  The config file for the test {pipeline_type} is located at `/scripts/pipelines/{provider}/templates/test/test-pipeline.cfg`.
...
--build-pipeline-name quarkus-project-build 
```
...
--build-pipeline-name angular-project-build 
```
