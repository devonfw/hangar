...
-v, --image-tag-file           [Required, if language is not node or quarkus] Path to file containing the app's version number to be used as docker image tag. eg package.json for node, pom.xml for quarkus
...
```
./pipeline_generator.sh -c ./templates/package/package-pipeline.cfg -n node-project-package -l node -d C:/Users/$USERNAME/Desktop/node-project --build-pipeline-name node-project-build --quality-pipeline-name node-project-quality -i username/node-project -v pom.xml -u username -p password -b develop -w
```
