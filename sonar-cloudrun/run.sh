...
if [[ "$1" = '/opt/sonarqube/bin/sonar.sh' ]]; then
    ...
    chmod -R 777 "${SQ_DATA_DIR}" "${SQ_EXTENSIONS_DIR}" "${SQ_LOGS_DIR}" "${SQ_TEMP_DIR}" 2>/dev/null || :

    # Allow the container to be started with `--user`
    if [[ "$(id -u)" = '0' ]]; then
        #chown -R sonarqube:sonarqube "${SQ_DATA_DIR}" "${SQ_EXTENSIONS_DIR}" "${SQ_LOGS_DIR}" "${SQ_TEMP_DIR}"
        chown -R fuse:fuse "${SQ_DATA_DIR}" "${SQ_EXTENSIONS_DIR}" "${SQ_LOGS_DIR}" "${SQ_TEMP_DIR}"
        # usermod -a -G fuse sonarqube
        addgroup -S fuse
        addgroup sonarqube fuse
        echo "Dropping Privileges"
        exec su-exec sonarqube "$0" "$@"
    else
        echo "Mounting GCSFuse"
        echo "With user $(id -u):$(id -g)"
        echo "Mounting GCS Fuse."
        gcsfuse --foreground --debug_gcs --debug_fuse --debug_http -o allow_other $BUCKET $SQ_DATA_DIR
